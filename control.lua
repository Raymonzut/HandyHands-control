...
function change(event, positive)
    ...
    elseif positive == false and d.settings[item] == 0 then
        if item == 'Default' then
            ...
            d.settings[item] = nil
            printOrFly(p, 'Your HandyHands setting for [item='..item..'] was deleted [Default is '..d.settings['Default']..' stacks]')
        end
    end
    global.players_data[event.player_index] = d
end
    ...
    if wasnil or force then
        ...
        if wasnil then
            ...
            game.players[player_index].print(
                'HandyHands autocrafting enabled for quickbar filtered items and ammo. Default amount: '..global.players_data[player_index].settings['Default']..' stacks.'
            )
